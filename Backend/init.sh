...
for i in bin etc/alternatives lib lib64 usr/bin usr/include usr/lib usr/share etc/java-8-openjdk #usr/lib64 usr/libexec
do
	mkdir -p "$i"
	chmod $MODE0 "$i"
...
setcap cap_fowner,cap_kill,cap_setuid,cap_setgid,cap_sys_resource,cap_sys_chroot+ep "${0%/*}/safeJudger"
